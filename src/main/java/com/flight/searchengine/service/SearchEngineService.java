...
public class SearchEngineService{
	...
	public void insertDataFromCsv() {
		...
		public List<FlightDetails> getAvailableFlights(String source,String destination,Timestamp departure,int price,long duration,String flightName,String offercode){
	        return repo.findAll(new Specification<FlightDetails>() {
	            ...
	            public Predicate toPredicate(Root<FlightDetails> root, CriteriaQuery<?> query, CriteriaBuilder criteriaBuilder) {
	                ...
	                if(departure!=null) {
	                    predicates.add(criteriaBuilder.and(criteriaBuilder.equal(root.get("departure"), departure)));
	                }
	                if(price!=0){
	                    predicates.add(criteriaBuilder.and(criteriaBuilder.equal(root.get("price"), price)));
	                }
	                if(duration!=0){
	                    predicates.add(criteriaBuilder.and(criteriaBuilder.equal(root.get("duration"), duration)));
	                }
	                if(flightName!=null) {
